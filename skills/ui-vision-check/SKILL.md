...
### Step 3: AI Vision 分析（项目 Vibe 专属维度）

**先从 `docs/ui/UI_SHOWCASE.md` §Vibe + §Anti-default note 加载本项目的基底声明**——以下 4 个维度的"目标值"全部从项目声明派生,**不要套用 FlameTree 默认母语**(暖陶瓷/施釉陶/暖米色)除非项目 §Vibe 显式继承。

对每张截图进行以下 4 个维度的感知分析：

#### 3.1 材质与光影感知 (Material & Lighting)
- 是否呈现项目 §Vibe 声明的核心质感？(项目说"施釉陶艺" → 审视温润感;项目说"冷峻终端" → 审视冷光锐利感;项目说"报纸 editorial" → 审视纸感哑光)
- 阴影/背景/光影是否与项目 §Vibe 的环境色一致？(暖陶项目预期暖底如 `#FFF8F0` + 焦糖弥散阴影;冷峻项目预期可能完全不同——以**项目自己声明**为准)
- 有无破坏项目视觉隐喻的元素？(参考项目 §Anti-default note 列的反例)
...
> （目标：从项目 `docs/ui/UI_SHOWCASE.md` §Vibe 段加载本项目声明的核心隐喻——禁止套用 FlameTree 默认母语"早晨阳光洒在早餐桌上的陶瓷餐具",除非项目 §Vibe 显式继承）
