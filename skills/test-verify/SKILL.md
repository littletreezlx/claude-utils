...
4. **熔断**：同一变异点修复 2 次仍无法 catch → 标记 `[ESCALATION]` AI 升级调试策略（代码设计评审/重构后重试），仍无法解决则调 `/think --quick` 决策（转 TODO 或丢弃），`/think` 也无法决策才写入 to-discuss.md
...
### 激进执行约束（反保守派，防止变异被跳过 / 盲点被转 TODO）

> **核心原则**：Tier 分级是 Token 成本的节流阀，不是"下次再说"的借口。变异清单一旦输出，每一条都必须执行；ESCAPED 一旦发现，默认当场修复。

**必做清单（禁止转 TODO 的动作）**：

| 动作 | 何时强制执行 | 为什么不能转 TODO |
|------|------------|-----------------|
| Tier 1 全量变异 | 目标文件命中 auth/payment/security/permission/token/state-machine/order 等关键词 | 这些文件的测试盲点 = 生产事故，不做等于没跑 |
| 增量模式关键词升级 | `git diff` 命中上述关键词 → 自动升 Tier 1 | 改过的关键路径必须验，不能按默认 Tier 跳过 |
| ESCAPED 当场修复 | Step 3 每个 ESCAPED 变异点默认直接改测试 | 修复方案已在 Step 3 生成,延迟修只会变成无主 bug |
| 变异清单执行率 100% | Step 1 输出 3 条就执行 3 条，输出 5 条就执行 5 条 | 出完题跳题 = 违反"先出题后验证"铁律 |

**只有以下 2 类可以走例外路径**（报告中必须显式标注 `[ESCALATION]`）：

1. **熔断触发**：同一变异点修复 2 次仍无法 catch → 代码设计本身可能有问题,升级给用户或 `/think`
2. **测试基础设施异常**：测试跑不起来/假阳性（那是 `/test-audit` 的问题，不是本 skill 的职责）

**严格禁止的"转 TODO"借口**：

- ❌ "Tier 2 只抽样 5 个" → 抽样是上限不是下限，Tier 2 里命中关键词的**必须升 Tier 1**
- ❌ "变异点太多，这次先做一半" → 单文件上限就是 5 个，5 个都做不完说明上限定错了，缩小 scope 而不是跳题
- ❌ "ESCAPED 先记录，后续统一修" → 修复方案已经在 Step 3 里了，当场修
- ❌ "测试通过了，说明测得挺好" → 这是覆盖率思维；红队的工作是 **证伪**，不是确认
- ❌ 自创 P0/P1 分级把部分变异延迟执行 → skill 无分级概念，Tier 就是 Tier

**每次报告的完成标志**：变异清单中每一条要么 `🟢 CAUGHT` / `🔴 ESCAPED → ✅ 已修复` / `🔴 ESCAPED → ⚠️ ESCALATION`，禁止出现"待补"、"下次处理"、"暂缓"等模糊状态。
