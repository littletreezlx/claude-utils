...
```bash
# 动态发现端口（从 debug_server.dart 读取，禁止硬编码）
...
# 验证 /cyborg/tap 可用
curl -s 'localhost:$PORT/cyborg/tap?nodeId=0' | python3 -c "import sys,json; print(json.load(sys.stdin))"
```
...
curl -s localhost:$PORT/cyborg/dom | python3 -m json.tool
```
...
```bash
curl -s localhost:$PORT/state/route     # 当前在哪个页面
curl -s localhost:$PORT/state/overlays  # 是否有弹窗
```
...
curl -s 'localhost:$PORT/cyborg/tap?nodeId=12'
# → {"ok":true,"data":{"action":"tap","nodeId":12,"status":"dispatched"}}

# 长按
curl -s 'localhost:$PORT/cyborg/longPress?nodeId=12'

# 文字输入（直接注入到 TextField，绕过键盘）
curl -s 'localhost:$PORT/cyborg/input?nodeId=7&text=hello'

# 滚动
curl -s 'localhost:$PORT/cyborg/scroll?nodeId=8&direction=down'
```
...
curl -s 'localhost:$PORT/cyborg/tap?x=200&y=300'
```
...
curl -s localhost:$PORT/state/route      # 页面是否切换
curl -s localhost:$PORT/state/overlays   # 弹窗是否出现/消失
curl -s localhost:$PORT/state/management # 数据是否变化（如适用）
curl -s localhost:$PORT/data/groups      # DB 层数据验证（如适用）
```
