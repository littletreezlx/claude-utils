...
- **Cyborg = 皮（Vision+Tap），Oracle = 骨（State 验证）** — 截图感知 UI，cliclick 驱动交互，curl 验证状态。三层配合，缺一不可
- **同源坐标系** — 感知（screencapture）和行动（cliclick）使用同一坐标系（macOS 屏幕点），消除隐藏变量
...
| 条件 | 模式 | 探索能力 |
|------|------|---------|
| cliclick + Simulator + Debug Server 全部就绪 | **Cyborg** | 真实 UI 交互 + 导航 + 手势 + State 验证 |
| 仅 Debug Server 可用 | **Fallback** | 端点覆盖测试（诚实声明：探索范围 = 端点数量，无法覆盖导航/手势/动画） |
...
当 Simulator 或 cliclick 不可用时自动降级。
...
5. **点击精度有限** — 小按钮可能需要多次尝试，结合 State Oracle 辅助判断
