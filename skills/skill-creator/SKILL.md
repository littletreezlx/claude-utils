...
| **祈使语气** | 指令用祈使句，简洁直接 |
| **不说显而易见的事** | Claude 对编程已经很在行，Skill 应聚焦于打破 Claude 常规思维的信息——反直觉的约定、项目特有的坑、Claude 容易犯错的地方 |
| **必须有踩坑点章节** | 每个 Skill 应有 `## Gotchas / 踩坑点` 章节，记录 Claude 使用该 Skill 时的常见失败模式，随使用持续积累 |
| **留灵活性，不限制太死** | 给信息而非死板指令，让 Claude 能适应具体场景。Skill 复用性强，指令太具体会在其他场景产生副作用 |
