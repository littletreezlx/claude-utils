...
> **核心场景**：规划用高智能模型（Claude Opus），执行用性价比模型（GLM4.7 等）
>
> 执行模型智能较低，但遵从命令。任务描述必须**完整自包含**，不依赖执行者的"理解能力"。

### 设计原则

| 原则 | 说明 | 示例 |
|------|------|------|
| **自包含** | 每个任务独立可执行，不依赖其他任务的上下文 | 不要写"同上"、"参考前一任务" |
| **显式化** | 所有隐含假设都要写明 | 不要假设执行者知道项目用什么框架 |
| **具体化** | 用精确路径代替模糊描述 | `src/auth/login.ts` 而非 "认证模块" |
| **可验证** | 完成标准必须客观可检验 | "测试通过" 而非 "代码质量良好" |
...
**🏠 项目背景**：
[2-3 句话说明这个项目是什么，让完全不了解项目的执行者能快速理解]
...
**📁 核心文件**：
- `src/xxx/target.ts` - [需要修改，添加 XXX 功能]
- `src/xxx/related.ts` - [需要修改，调用新功能]
- `src/xxx/reference.ts` - [仅参考，不修改，查看现有实现模式]
...
| 字段 | 必填 | 说明 |
|------|------|------|
| **🏠 项目背景** | ⭐ 推荐 | 执行者可能完全不了解项目，需要快速建立认知 |
| **🎯 任务目标** | ✅ 必填 | 一句话说明最终要达成什么 |
| **📁 核心文件** | ✅ 必填 | 精确路径 + 操作类型（修改/参考/新建） |
| **🔨 执行步骤** | ⭐ 推荐 | 3-5 步具体操作，不要太细也不要太粗 |
| **✅ 完成标志** | ✅ 必填 | 客观可验证的标准，最好是 checklist 格式 |
| **⚠️ 注意事项** | ❌ 可选 | 常见坑点，避免执行者踩雷 |

### 项目背景的复用

**方案 1：每个任务都写**（推荐用于跨 STAGE 或任务较少时）
...
**🏠 项目背景**：
电商后台管理系统，使用 NestJS + TypeORM + PostgreSQL。
项目结构遵循 NestJS 标准模块化设计，每个功能模块在 src/modules/ 下。
...
```

**方案 2：STAGE 头部写一次**（推荐用于同一 STAGE 内任务紧密相关时）

```markdown
## STAGE ## name="用户模块开发" mode="parallel"

> **🏠 项目背景**：电商后台管理系统，NestJS + TypeORM + PostgreSQL。
> 本阶段所有任务都在 src/modules/user/ 目录下工作。
> 参考 src/modules/product/ 的实现模式。

## TASK ##
实现用户登录 API
[可以省略项目背景，因为 STAGE 头部已说明]
...
...
**🏠 项目背景**：
Node.js 后端服务，使用 Express + TypeScript。
API 错误需要统一返回 `{ code, message, data }` 格式。
项目已有 `src/common/errors.ts` 定义了标准错误类。

**🎯 任务目标**：
用户模块的所有 API 错误使用统一的错误处理中间件，返回标准格式。

**📁 核心文件**：
- `src/modules/user/user.controller.ts` - [修改] 移除 try-catch，使用错误中间件
- `src/modules/user/user.service.ts` - [修改] 抛出标准错误类
- `src/common/errors.ts` - [参考] 查看已有的错误类定义
- `src/middleware/error-handler.ts` - [参考] 查看错误处理中间件实现

**🔨 执行步骤**：
1. 阅读 `src/common/errors.ts` 了解项目的标准错误类
2. 阅读 `src/middleware/error-handler.ts` 了解错误处理流程
3. 修改 `user.service.ts`，将 `throw new Error()` 改为 `throw new BusinessError()`
4. 修改 `user.controller.ts`，移除 try-catch 块（让错误冒泡到中间件）
5. 运行测试验证

**✅ 完成标志**：
- [ ] user.controller.ts 中无 try-catch 块
- [ ] user.service.ts 中所有 throw 使用 BusinessError 或其子类
- [ ] `npm test -- user` 全部通过

**⚠️ 注意事项**：
- 不要修改 `src/common/` 目录下的文件
- 保持现有 API 的响应结构不变
