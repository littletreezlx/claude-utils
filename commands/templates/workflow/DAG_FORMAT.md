...
**所有任务产物聚合在 `.task-{命令名}/` 目录内**，清理时整个目录一刀切。

| 文件 | 位置 | 说明 |
|------|------|------|
| `.task-{命令名}/dag.md` | 项目根目录下的隐藏目录 | **入口文件**，batchcc 执行的起点 |
| `.task-{命令名}/*.md` | 同目录 | 任务细节文件（按需拆分） |
| `.task-{命令名}/state.json` | 同目录 | 执行状态（batchcc 运行时自动生成） |

### 调用方式

`batchcc` 接收命令名（无 `.` 前缀的短名即可），自动查找 `.task-{命令名}/dag.md`：

```bash
batchcc task-xxx              # 自动解析到 .task-xxx/dag.md（推荐写法）
batchcc .task-xxx             # 等价
batchcc .task-xxx/dag.md      # 显式路径
```

### 命名约定

**规则**：目录 = `.task-{命令名}/`，入口文件 = `dag.md`

| 命令 | 入口文件路径 |
|------|-------------|
| `/refactor-project` | `.task-refactor-project/dag.md` |
| `/test-plan` | `.task-test-plan/dag.md` |
| `/comprehensive-health-check` | `.task-comprehensive-health-check/dag.md` |
| `/refactor-module` | `.task-refactor-module-[模块名]/dag.md` |
| `/todo-huge-task` | `.task-{用户指定名}/dag.md` |
| `/doc-update-context` | `.task-doc-update-context/dag.md` |
| `/doc-quality-review` | `.task-doc-quality-review/dag.md` |
...
>
> **⚠️ 关键约束**：收尾 TASK 运行时**没有前序任务的会话历史**（batchcc 每个任务是独立会话）。必须通过 git diff/log 和文件系统自行发现前序任务的产出，不能依赖会话记忆。
...
| `### TASK` 或 `## TASK:` | `## TASK ##` |
| 入口文件放在项目根 `task-xxx` | 入口文件放在 `.task-xxx/dag.md` |
| 入口文件命名 `task-xxx.md` | 入口文件固定命名 `dag.md` |
| 任务目录不加 `.` 前缀（`task-xxx/`） | 目录用 `.` 前缀隐藏（`.task-xxx/`） |
| state 散落在项目根 `task-xxx.state.json` | state 在 `.task-xxx/state.json`，随目录聚合 |
| 缺少 `文件:` 字段 | 并行任务必须有 `文件:` |
