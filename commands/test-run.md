...
| setup 复杂 | 单个测试需大量前置条件 | 记录，建议简化接口或使用 Test Facade |
| 无法隔离 | 测试一个功能必须牵扯多模块 | 记录，建议解耦 |
| 断言困难 | 内部状态不可观测 | 记录，建议暴露关键状态 |
| **E2E 过于脆弱** ⭐ | UI 状态变化导致测试失败 | 考虑使用 Test Facade 绕过 UI |
| **E2E 过于缓慢** ⭐ | 大量时间花在准备数据上 | 考虑应用冰山策略（转移逻辑验证到单元测试） |

**处理流程**：
- 局部问题 → 立即小重构 → 继续修复测试
- 架构问题 → 完成当前任务 → 在输出中列出优化建议
- **E2E 脆弱/缓慢** → 考虑应用冰山策略（转移逻辑验证到单元测试）
