...
1. **项目定位**
   - README.md / ABOUT.md - 项目是做什么的？
   - package.json / pubspec.yaml - 项目名称和描述
...
**输出：功能清单**（内部使用，不输出给用户）：
```
🔴 关键路径（必须有测试）：用户认证、核心业务流程、支付/交易
🟡 重要功能（应该有测试）：数据处理、状态管理、API 集成
🟢 辅助功能（可选测试）：工具函数、配置、日志
```
...
**测试覆盖差距分析**：
- 对照第一步的功能清单，哪些 🔴🟡 功能没有测试？
- 现有测试质量如何？（flaky？超时？隔离问题？）
