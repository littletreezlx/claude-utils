...
TODO.md 更新完成后，**自动生成**机器状态快照，提升下次恢复效率。

### 生成 snapshot.md

创建 `.claude/context/snapshot.md`，记录当前环境状态：
...
# 生成快照内容
cat > .claude/context/snapshot.md << 'SNAPSHOT_EOF'
# 🔄 Session Snapshot
Generated: [当前时间 YYYY-MM-DD HH:MM]

## Git Status
```
[运行 git status --short 的输出]
```

## Recent Changes
```
[运行 git diff --stat HEAD~1 的输出，最多20行]
```

## Modified Files (last 24h)
```
[最近24小时修改的文件列表，排除 node_modules/.git 等]
```
...
如果本次会话做了重要技术决策，追加到 `.claude/context/decisions.md`：

```markdown
- [YYYY-MM-DD] [模块名] 决策：XXX | 原因：XXX | 影响文件：XXX
```

### ⚠️ 注意事项
- `snapshot.md` 由机器自动生成，比 AI 手写更准确
- **不要把 decisions.md 的内容复制到 TODO.md**，保持 TODO.md 独立可执行
- snapshot.md 是补充信息，TODO.md 仍是执行的唯一依据
