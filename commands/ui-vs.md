...
3. **Claude Design 在线 share URL**：用户贴 URL 时，用 WebFetch 读 HTML(但仍以源码为主，不截图)
4. **临时下载目录**：用户刚从 Claude Design export 还没归档的目录

若找不到 bundle → 停止并提示用户："请指定 Claude Design export 目录路径，或先 export 后再跑 /ui-vs"。
