...
- ~~Onboarded commit~~ → 固定填 `n/a`(默认工作流不追踪此字段,见 Phase 4.1 模板说明 + `/ui-adopt` Phase 5 默认跳过的逻辑)
...
- **Onboarded from commit**: `n/a` — 默认工作流: 每次使用 Claude Design 时即时让它读最新 codebase, 不预先 onboard, 故不维护此字段。如本项目改用追踪模式(预先 onboard + 维护字段), 把此处改成真实 commit sha 即可,`/ui-adopt` Phase 5 会自动启用 re-onboard 提示
- **Onboarded from UI_SHOWCASE mtime**: 2026-04-24 (bootstrap 时的 mtime)
- **Re-onboard trigger**: 不适用(默认工作流); 如启用追踪模式, UI_SHOWCASE.md §Invariants 发生变化时或距 onboard 超过 30 天即触发
...
- EXTERNAL_REFS 里 `onboarded_commit = n/a`(默认工作流不追踪),无需回填。如改用追踪模式见 EXTERNAL_REFS 模板说明
```
...
- **onboarded_commit 固定填 `n/a`**,不询问用户(默认工作流不追踪此字段;改用追踪模式时手动改 EXTERNAL_REFS 即可)
