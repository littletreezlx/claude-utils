...
**如果存在 `.claude/context/snapshot.md`**，快速浏览获取补充信息：
...
```bash
# 检查是否存在
if [ -f .claude/context/snapshot.md ]; then
    # 读取快照，获取环境状态
fi
```

**如果存在 `.claude/context/decisions.md`**（做架构变更时）：
- 了解过去的技术决策
- 避免推翻正确的历史决定

⚠️ **注意**：
- snapshot.md 只是补充信息，**主要执行依据仍是 TODO.md**
- 如果这些文件不存在，直接跳过，继续执行
- 不要因为缺少 snapshot 而停止
