...
#### 常见调试陷阱
- **过度日志陷阱**：每个 chunk 记录日志，掩盖关键信息
- **方向错误陷阱**：在 UI 层排查数据传输问题
- **组件复杂陷阱**：TypewriterText 同时处理流式更新 + 动画

#### 分层诊断原则 🔍
遇到问题时按以下顺序：
1. **数据层**：数据是否正确接收？（Network 日志）
2. **状态层**：状态是否正确更新？（关键状态日志）
3. **UI层**：渲染是否正确？（避免过多渲染日志）
4. **动画层**：特效是否正确？（优先保证功能）
...
#### 组件设计原则 🏗️
- **功能优先**：先保证可用，再添加动画效果
- **单一职责**：组件只负责自己的状态，不要跨界
- **降级方案**：动画失败时要有备选渲染方案
- **异步安全**：Stream/Generator 要有明确的结束机制
