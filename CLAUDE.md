...
| 不确定时 | 询问用户，不要猜测 |
| 定期保养 | `/codebase-align` → `/comprehensive-health-check`（按需） → 🔴区域修复 |
...
6. **依赖审慎**：可自主添加轻量依赖（如 google_fonts、工具类包），但需在提交信息中说明新增依赖及理由。重型框架或架构级依赖（如状态管理、数据库、网络框架）仍需用户确认。
...
**跑偏时优先新开会话**：方案连续修补仍不理想时，`git checkout` 回滚代码，新开会话重来。当前会话的错误上下文会污染后续判断，推倒重来比硬推效果好。
...
2. 运行测试（确保起点干净）
3. **先读测试**理解预期行为，再读实现代码（测试是可执行的行为规范）
...
项目可创建 `.claude/SNIPPETS.md` 存放代码模式速查（正例 + 反例），供 AI 快速理解项目惯例。
