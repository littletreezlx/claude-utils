#!/usr/bin/env python3
"""
批量执行 Codex 命令的脚本 - 支持 DAG 格式

## 用途
从 template 文件中提取任务并用 Codex 批量执行，支持：
...
# 指定模板文件
python batchcx.py task-refactor.md

# 并行执行（默认8个线程）
python batchcx.py --parallel 4

# 强制串行执行
python batchcx.py --single

# 查看执行计划（不实际执行）
python batchcx.py --dry-run

# 重新开始（清空状态文件）
python batchcx.py --restart
```

## 文档参考
- tools/batch/docs/DAG_FORMAT.md: DAG 格式规范
- AGENTS.md: 维护指南
"""
...
class CodexBatchExecutor(BaseBatchExecutor):
    ...
    def __init__(self):
        ...
        self.codex_bin = (
            shutil.which("codex")
            or os.environ.get("_CODEX_REAL_BIN")
            or "/Users/zhanglingxiao/.nvm/versions/node/v22.18.0/bin/codex"
        )
        if not self.codex_bin or not os.path.exists(self.codex_bin):
            print("❌ 未找到 codex 命令，请确认已安装并在 PATH 中")
            print("修复：将 codex 加入 PATH，或设置 _CODEX_REAL_BIN 指向真实可执行文件")
            sys.exit(1)
    ...
    def _auto_commit_if_needed(self, task_description: str, task_id: int = None):
        ...
        - **排除 batchcx 任务过程产物**（.task-*/、task-* 裸文件、state.json）—
          这些是 batchcx 运行时的临时编排/状态文件，不该形成独立 commit；
        ...
        """
        try:
            # 1. add 业务文件，显式排除 batchcx 任务过程产物
...
"""
    ...
    def build_command(self, task_description: str) -> str:
        ...
        Returns:
            codex命令字符串
        """
        # 转义内部的双引号，避免命令展示和 fallback shell 执行时解析错误。
    ...
    def execute_command_parallel(self, args: Tuple[int, str, str]) -> TaskResult:
        ...
        try:
            # 提取 codex exec 命令中的内容并转换为完整路径命令。
            prefix = 'codex exec "'
            suffix = '" --skip-git-repo-check --yolo'
            if command.startswith(prefix) and command.endswith(suffix):
                content = command[len(prefix):-len(suffix)]
...
def resolve_task_entry(arg: str) -> Optional[Path]:
    ...
    1. `batchcx task-xxx`          → 查找 `.task-xxx/dag.md`（新格式主路径）
    2. `batchcx .task-xxx`         → 等价于 1
    3. `batchcx .task-xxx/dag.md`  → 直接使用
    4. `batchcx some-file.md`      → 简单 TASK 格式（向后兼容）
    5. `batchcx some-file`         → 自动补 `.md`（向后兼容）
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='批量执行 Codex 命令 - 支持 DAG 格式')
    ...
    if args.template:
        ...
        if resolved is None:
            ...
            print("支持的写法:")
            print("  batchcx task-xxx            # 自动解析到 .task-xxx/dag.md")
            print("  batchcx .task-xxx           # 等价")
            print("  batchcx .task-xxx/dag.md    # 显式路径")
            print("  batchcx some-file.md        # 简单 TASK 格式")
            return 1
